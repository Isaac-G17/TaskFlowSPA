...
import { confirmDeleteTask, showError, success } from "../../utils/alerts";
    ...
    <article class="rounded-3xl border border-blue-100 bg-white p-6 shadow-lg shadow-blue-50">
      <div class="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
        ...
        <div class="flex gap-3">
          ...
          <button type="button" data-delete-task="${task.id}" data-delete-title="${task.title}" class="rounded-full border border-blue-200 px-4 py-2 text-sm font-semibold text-blue-700 hover:bg-blue-50">Eliminar</button>
        </div>
      </div>
    </article>`;
}
  ...
  try {
    ...
    list.querySelectorAll("[data-delete-task]").forEach((button) => {
      button.addEventListener("click", async () => {
        ...
        const taskTitle = button.dataset.deleteTitle || "esta tarea";

        if (!taskId) {
          return;
        }

        const confirmed = await confirmDeleteTask(taskTitle);
