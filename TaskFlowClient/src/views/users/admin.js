    ...
    <div class="rounded-2xl bg-blue-50 p-4" data-user-id="${user.id}">
      <div class="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        ...
        <div class="flex flex-wrap items-center gap-2">
          ...
          <button type="button" data-delete-user="${user.id}" data-delete-title="${fullName}"class="rounded-full border border-blue-200 px-3 py-1 text-xs font-semibold text-blue-700 hover:bg-white">Eliminar</button>
        </div>
      </div>
    </div>`;
}
...
export async function setupAdminView() {
    ...
    usersList.querySelectorAll("[data-save-role]").forEach((button) => {
      button.addEventListener("click", async () => {
        ...
        try {
          ...
          success(`Rol de ${user.name} ${user.lastname} actualizado a ${newRole}.`);
        } catch (error) {
  ...
  button.addEventListener("click", async () => {
    ...
    const userTitle = button.dataset.deleteTitle || "este usuario";

    if (!userId) return;

    const confirmDelete = await confirmDeleteUser(userTitle);
    ...
    try {
      ...
      // actualizar contador
      usersCount.textContent = String(
        Number(usersCount.textContent) - 1
      );
